...
const setPoints = (points:TypedArray, p:Vec2|Vec3, i:number) => {
  ...
  const calculateNormal = (vertices:Vec3[]) => {
    const v0 = vertices[0]
    const v1 = vertices[1]
    const v2 = vertices[2]
  ...
  const CSGOutlines2LineSegmentsVertices = (key:string) => (csg:any) => {
  ...
  function CSGCached (func:Function , data, cacheKey, transferable, unique, options) {
  ...
  export function JscadToCommon (csg:any, transferable, unique, options) {
