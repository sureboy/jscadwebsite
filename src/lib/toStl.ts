import flatten from '@jscad/array-utils/src/flatten'; 
import toArray from '@jscad/array-utils/src/toArray';  
import modeling from '@jscad/modeling';  

const { geometries,modifiers } = modeling  

export const mimeType = 'application/sla' 
 
//const observed = serializer.serialize({ binary: true }, object1)
export const serialize = (options:Object, ...objects:Object[]) => {
    const defaults = {
      binary: true,
      statusCallback: null
    }
    ...
    //geometries.geom3. 
    objects3d = toArray( modifiers.generalize.generalize.prototype({ snap: true, triangulate: true }, ...objects3d ))
  
    return  serializeBinary(objects3d, options) 
  }
  


  const serializeBinary = (objects:any, options:any) => {
    ...
    let numtriangles = 0
    let numpolygons = 0
    objects.forEach((object:any, i:number) => {
