...
import { GCodeParser,  LayerType } from "./parser";
import type {LayerDefinition} from "./parser";
import type { SegmentColorizer } from "./SegmentColorizer";
...
export class GCodeRenderer {
  ...
  public fitCamera() {
    ...
    this.camera.position.x =
      (this.parser.min?.x || 0) +
      ((this.parser.max?.x || 0) - (this.parser.min?.x || 0)) / 2;
    const fov =  this.camera.fov*(Math.PI /180); 
    const size = boundingBox.getSize(new Vector3()).length()
    this.camera.position.z = size /2/Math.tan(fov/2); 
